...
import com.theicecreambear.handlers.SoundHandler;
...
public class Engine {
	...
	private void checkSoundKeys() {
		if (hHandler.isKeyDown(KeyEvent.VK_ESCAPE)) {
			System.exit(-1);
		}
		if (hHandler.isKeyDown(KeyEvent.VK_W)) {
			y -= 1;
		}
		if (hHandler.isKeyDown(KeyEvent.VK_S)) {
			y += 1;
		}
		if (hHandler.isKeyDown(KeyEvent.VK_A)) {
			x -= 1;
		}
		if (hHandler.isKeyDown(KeyEvent.VK_D)) {
			x += 1;
		}
	}
}